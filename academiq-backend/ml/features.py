...
def extract_features_for_course(course_id: int, db: Session) -> list:
    """
    Extract features from early performance simulating the early alert point.
    """
    assessments = db.query(Assessment).filter(Assessment.course_id == course_id).all()
    if not assessments:
        return []

    # Filter for early alert (e.g., Week 1-6/14)
    early_assessments = [a for a in assessments if (a.week or 0) <= 14]
    ...
    # FIX 1: Define the 'students' variable (Fixes the NameError)
    ...
    for student in students:
        ...
        for i, clo in enumerate(clos):
            ...
            # Identify which specific COs are underperforming
            if pct < 60.0:
                at_risk_list.append(f"CO{i+1}")

        # Pad to exactly 4 CLO features for model consistency
        ai_clo_features = list(clo_pcts)
        while len(ai_clo_features) < 4:
            ai_clo_features.append(0.0)
        ai_clo_features = ai_clo_features[:4]
        ...
        # This 'at_risk' variable is the "y" target for training
        ...
        features.append({
            ...
            "at_risk": at_risk,           # FIX 2: Added to resolve "Input y contains NaN"
            "at_risk_cos": at_risk_list,  # Added for frontend alerts
            **{f"clo{i+1}_early_pct": pct for i, pct in enumerate(ai_clo_features)}
        })
